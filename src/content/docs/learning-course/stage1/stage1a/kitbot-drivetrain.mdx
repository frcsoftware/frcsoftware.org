...
<Aside type="note">
    ...
    button that allows you to quickly import the class. The import statements
    for `TalonFX` and `SparkMax` look like `import
...
For the left motors, the invert setting will be `true` for REV code and `InvertedValue.CounterClockwise_Positive` for CTRE code.
...
Instead they should be configured with an invert setting of `false` or `InvertedValue.CounterClockwise_Positive` so they also drive the robot forward when given a positive input.
