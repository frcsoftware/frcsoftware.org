...
>
    Photo Credit: FIRST's Blog post
</ContentFigure>
...
This stages uses two types of motor controllers: REV's SparkMAX and CTRE's TalonFX

<ContentRow>
    <ContentFigure
        src="/learning-course/stage1/stage1a/sparkmax.webp"
        alt="A photo of a sparkmax"
        width="30%"
    >
        Photo Credit: REV
    </ContentFigure>
    <ContentFigure
        src="/learning-course/stage1/stage1a/kraken.webp"
        alt="A photo of the built in talonfx motor controller"
        width="30%"
    >
        Photo Credit: CTRE
    </ContentFigure>
...
<Aside type="note">
    The `TalonFX` and `SparkMax` classes, like many other classes that will be
    used, come from external libraries provided by their respective vendors.
    Therefore, these classes need to be imported into files where they are used
    so the compiler knows what they are. If you use a class that has not yet
    been imported VS Code will mark it as an error and provide a quick fix
    button that allows you to quickly import the class. The import statement for
...
<Tabs syncKey={REV_CTRE_CHOOSER_KEY}>
    <TabItem label = "CTRE">
        ...
        CTRE uses a `CANBus` that is constructed with a `CANPort enum to specify which of Systemcore's CANBuses the device is connected too.
        This enum is a field in the `CANPort` class and only CAN_S0 through CAN_S4 will ever be used since Systemcore only has five CAN Busses.
    ...
    <TabItem label = "REV">
        ...
        REV uses a `CANPort` enum to specify which of Systemcore's CANBuses the device is connected too.
        This enum is a field in the `CANPort` class and only CAN_S0 through CAN_S4 will ever be used since Systemcore only has five CAN Busses.
...
<Aside type="answer" title="Solution">
    <Tabs syncKey={REV_CTRE_CHOOSER_KEY}>
        <TabItem label="CTRE">
