...
Debugging is the process of finding errors, often called bugs, then fixing them.
This section will cover effective strategies to debug your robot code.
...
For example, if we misspell a variable name,
putting the cursor over the error shows `numer cannot be resolved to a variable`.
...
One common type of runtime error is dividing by zero.
...
However, if the code was to run, it would give a runtime error for dividing by zero.
...
This happens in `int answer = 10/0;` because it's dividing by zero.
To fix this error, the code needs to be changed so it never divides by zero.
...
- VS Code has a bunch of accessibility features which can be helpful for debugging and writing code.
