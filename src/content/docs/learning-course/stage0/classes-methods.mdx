...
The result of `minus` isn't a location on the field.
It's a **vector**, an arrow with a direction and a length, like "3 meters right and 4 meters up."
Subtracting `other` from this `Point` gives the vector that points from `other` to this `Point`.
A `Point` can hold a vector as well as a location, since both are an `x` and a `y`.

Finally, we're going to define a `norm` method that returns the distance of this `Point` from the origin.
Picture a right triangle with one corner at the origin and the opposite corner at this `Point`.
Its two shorter sides are `x` and `y` long, so by the Pythagorean theorem, its longest side is the square root of `x² + y²`.
For the point (3, 4), that's the square root of 9 + 16, which is 5.
The length of a vector is called its **norm**, which is where the method gets its name.
...
Immutability is useful because it makes classes easier to reason about, since you never have to worry about a value changing unexpectedly.
...
Now, you can dive straight into `Methods.java`!
