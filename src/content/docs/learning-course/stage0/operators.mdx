...
They help with incrementing and decrementing values, and are commonly used in loops, which are covered later in this stage.
For now, we can use the operators to change variables on their own.
...
<Aside type="note">
`==` and `!=` cannot be used to compare the values of `String`s. 
...
<Aside type="note">
    Like arithmetic operators, logical operators have an order of operations:
    `!` is applied first, then `&&`, then `||`. Parentheses can be used to
    change this order.
</Aside>
...
In the example, `e`'s value gets changed to 12 as `f` is equal to 2, so 2 + 10 is 12.
...
<Aside type="exercise">
    ...
    Now, you can dive straight into `Operators.java`, which you set up in the last section!
