...
<Aside type="note">
    Another type of loop is the do-while loop, but this will not be covered due
    to sparse use in FRC. However, if you want to learn more about do-while
    loops, you can do so through
...
Conditionals are used to run a block of code once, whereas a loop can run a block of code multiple times.

## while

`while` loops are the most simple type of loops.

The syntax of a `while` loop is as shown:
...
First, the code checks if the condition is true.
...
<Aside type="tip">
    ...
    [operator page](/learning-course/stage0/operators).
</Aside>
...
In this example, we want our drive train to run for 7 iterations of the loop.
...
`timer` now increments by 1 each time the loop is run, and the loop will stop after 7 iterations.
