...
<Aside type="hint">
    Hints to questions are hidden in a "Hint" dropdown like this one.
</Aside>

<Aside type="answer">
    Answers to questions are hidden in an "Answer" dropdown like this one.
</Aside>

FRCSoftware focuses on two different ways of writing code based on different motors.
In later stages, tabs will be used to accommodate for the differences.
To see code based on REV motors, click on the REV tab.
To see code based on CTRE motors, click on the CTRE tab.

<Tabs syncKey={REV_CTRE_CHOOSER_KEY}>
    <TabItem label="CTRE">Code for CTRE will be here</TabItem>
    <TabItem label="REV">Code for REV will be here</TabItem>
</Tabs>
