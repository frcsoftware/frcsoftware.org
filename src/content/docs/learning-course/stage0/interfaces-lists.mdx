...
The object it gives back isn't an `ArrayList`; it's a class inside the JDK that you'd never write out yourself.
That's fine, because you only use it through the `List` interface, the same way `isTooClose` only used `DistanceSensor`.

### List Mutability

In a [previous lesson](/learning-course/stage0/classes-methods/), we saw that some classes are **mutable** (their state can change after they're created) and some are **immutable** (it can't).
Lists work the same way, and since `List` is an interface, whether a particular list can change depends on which class implements it:

- An `ArrayList` is mutable: you can add, remove, and replace elements whenever you want.
- A list from `List.of` is immutable: its elements are fixed when it's created, and calling `add` or `remove` on it throws an `UnsupportedOperationException`.

Both are declared as `List`, so the compiler can't tell them apart; you only find out when the program runs.
